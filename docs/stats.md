...
| `service/relay_stats_seed.go` | 冷启动从历史日志重建 window summaries（`InjectSeedSummaries`），seeded 记录打标以便管理视图过滤 |
...
| GET | `/score_weights` | 获取渠道健康评分权重 |
| PUT | `/score_weights` | 更新渠道健康评分权重 |
...
| 接口 | 主要用途 | 关键字段 |
|------|----------|----------|
| `GET /api/relay/stats/` | 管理后台总览卡片；看进程启动以来的全局累计值 | `total_requests`, `success_requests`, `retry_recovery_rate`, `avg_duration_ms`, `avg_first_token_ms`, `avg_output_tps` |
| `GET /api/relay/stats/windows` | 管理后台最近窗口明细表；定位某个模型/渠道/分组最近 5 分钟表现 | `window_start`, `model_name`, `channel_id`, `group`, `success_attempts`, `excluded_attempts`, `avg_first_token_ms`, `channel_score` |
| `GET /api/relay/stats/timeseries` | 图表趋势；按模型/渠道/分组拆线看某个指标随时间变化 | `series[].points[].time`, `series[].points[].value`, `metric`, `group_by`, `interval` |
| `GET /api/relay/stats/dimensions` | 聚合透视；把多个窗口按维度重新汇总 | `data[key] = StatsCounters`，字段口径同 `/api/relay/stats/` |
| `GET /api/relay/stats/exclusion_rules` | 查看哪些错误会被视为“排除项”而不计入真实失败 | `model`, `channel_types`, `error_codes`, `status_codes`, `message_keywords`, `level`, `description` |
| `GET /api/relay/stats/score_weights` | 查看渠道健康分的权重配置 | `sync`, `async`, `level_weights` |
| `GET /api/relay/stats/models` | 用户侧模型统计；只看可见模型，不暴露渠道细节 | `model_name`, `success_rate`, `avg_duration_ms`, `p50_first_token_ms`, `tps`, `has_data` |
...
|------|------|
| `model_name` | 模型名 |
| `success_rate` | 最终请求成功率，按 request 维度而不是 attempt 维度计算 |
| `avg_duration_ms` | 模型维度的平均请求耗时，单位毫秒 |
| `p50_first_token_ms` | 模型维度的首字响应时间中位数（TTFT P50），仅流式请求参与；无流式数据时为 `null` / 省略 |
| `tps` | 输出 token 吞吐，基于 `completion_tokens / success_duration` 计算 |
| `total_requests` | 请求总数 |
| `success_requests` | 成功请求数 |
| `failed_requests` | 失败请求数 |
| `has_data` | 是否存在真实流量数据；为 `false` 时其余数值字段可为空 |

#### TTFT / FirstTokenDuration 口径

- `FirstTokenDuration` 只在流式输出请求上计算；非流式请求不会生成 TTFT 样本。
- 管理侧接口 `/`, `/windows`, `/timeseries`, `/dimensions` 暴露的是 `avg_first_token_ms`，口径是**平均值**，适合看窗口趋势和做渠道打分。
- 用户侧接口 `/models` 暴露的是 `p50_first_token_ms`，口径是**中位数**，比平均值更稳，不容易被个别慢请求拉偏。
- 当前实现中，实时内存窗口可以精确计算 `P50`；但 DB 持久化尚未保存 TTFT 样本，只保存 sum/count，所以进程重启后查询历史窗口时会退化为平均值近似。这一项如果要完全满足“始终返回中位数”，还需要继续完善持久化方案。
