...
7. **接口自解释**：所有 `/api/relay/stats/*` 接口都需要明确说明用途、查询参数、响应字段和字段口径，便于前后端联调和运维排障
8. **TTFT 采集口径明确**：`FirstTokenDuration` / TTFT 仅对流式输出请求计算；非流式请求不应伪造为 `0ms` 成功样本，而应保持为空
9. **模型指标使用中位数**：用户侧模型指标中的首字响应时间使用 `p50_first_token_ms`（中位数）而不是平均值，避免少量慢请求把模型观感拉偏
...
| 首字响应时间 | 流式请求从发出到收到第一个 token 的耗时；非流式请求不参与统计 |
| 成功率 | 请求最终成功率（用户视角）/ 单次尝试成功率（管理视角） |

同步模型指标对 TTFT 的查询口径需要区分两类接口：

- 管理侧聚合接口（`/`, `/windows`, `/timeseries`, `/dimensions`）返回 `avg_first_token_ms`，表示窗口聚合后的**平均** TTFT，适合看趋势和打分
- 用户侧模型接口（`/models`）返回 `p50_first_token_ms`，表示模型维度聚合后的**中位数** TTFT，适合展示给最终用户
...
综合以下因素计算渠道健康评分（0-100），权重完全可通过 `PUT /api/relay/stats/score_weights` 动态调整：

同步请求默认权重（`SyncScoreConfig`）：

```
base_weight     = 40   — 成功率基础分
severity_max    = 15   — 错误严重度扣分上限
recovery_weight = 5    — 重试恢复加分上限
speed_weight    = 25   — 首字响应速度加分上限
tps_weight      = 30   — 吞吐量（TPS）加分上限
```

速度分档（`speed_thresholds`）：≤500ms 满分 / ≤2s / ≤5s / ≤10s / >10s 0 分。

无有效数据时返回 `baseline_score`（默认 80），数据量少于 `sparse_threshold`（默认 5）时与基准值混合，避免单次偶发事件拉偏评分。

异步任务使用独立的 `AsyncScoreConfig`，分别考量提交成功率、提交速度、执行成功率和执行速度四个维度。

level 权重（L0=0, L1=1, L2=3, L3=6）与设计初稿一致。
...
- 当前 DB 持久化只保存 `FirstTokenCount` 和 `TotalFirstTokenNs`，不保存原始 TTFT 样本；因此 `/models` 在实时内存窗口上可返回严格 `P50`，但在进程重启后的历史窗口上会退化为平均值近似

## 6. 当前待完善

1. 如果希望 `/api/relay/stats/models` 在跨重启、跨实例聚合后仍严格返回中位数，需要在持久化层补充可合并的分位数摘要或样本持久化方案，而不是只保存 sum/count。
2. 管理侧 stats 接口虽然已有 handler 注释，但仍缺少统一的 OpenAPI/前端侧字段说明；仅看路由名时不够自解释，后续可以考虑补一份专门的 stats API 文档或前端 tooltip。
