...
type WindowSummary struct {
	...
	// FirstTokenSamples holds individual first-token durations (nanoseconds) collected
	// during this window. Populated only for in-memory windows; not persisted to DB.
	// Used by GetModelStats to compute P50 instead of a plain average. Because
	// samples are not persisted yet, DB-restored historical windows can only
	// provide an average-style fallback for model-level TTFT.
	FirstTokenSamples []int64 `json:"-"`
...
type windowBucket struct {
	...
	firstTokenSamples []int64 // capped at 1000 for P50 computation
...
func (wb *WindowBuffer) CollectAttempt(event *AttemptEvent) {
	...
	if event.FirstTokenDuration > 0 {
		ns := int64(event.FirstTokenDuration)
		b.totalFirstTokenNs += ns
		b.firstTokenCount++
		if len(b.firstTokenSamples) < 1000 {
			b.firstTokenSamples = append(b.firstTokenSamples, ns)
		}
	}
...
func buildSummaries(buckets map[windowBucketKey]*windowBucket, windowStart, windowEnd time.Time, windowSecs float64) []WindowSummary {
	...
	for key, b := range buckets {
		s := WindowSummary{
			...
			FirstTokenSamples:     b.firstTokenSamples,
