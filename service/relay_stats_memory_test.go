...
func TestGetModelStats_WithFirstTokenData(t *testing.T) {
	...
	assert.NotNil(t, s.P50FirstTokenMs, "P50FirstTokenMs should be non-nil when first-token data exists")
	assert.InDelta(t, 500.0, *s.P50FirstTokenMs, 10.0, "first token p50 ms should be ~500")
}
...
func TestGetModelStats_NoFirstToken_NilFirstTokenField(t *testing.T) {
	...
	stats := c.GetModelStats(0, 0)
	require.Len(t, stats, 1)
	assert.Nil(t, stats[0].P50FirstTokenMs, "P50FirstTokenMs should be nil when no streaming data")
}
...
func TestGetModelStats_HasData_False_ForZeroTrafficModels(t *testing.T) {
	...
	assert.Nil(t, zero.P50FirstTokenMs)
