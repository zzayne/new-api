...
func (m *MemoryStatsCollector) GetModelStats(startTime, endTime int64) []ModelStats {
	...
	type modelAgg struct {
		...
		firstTokenSamples     []int64 // for P50; nil for DB-restored windows
	...
	for _, w := range windows {
		...
		agg.firstTokenSamples = append(agg.firstTokenSamples, w.FirstTokenSamples...)
	...
	for model, agg := range buckets {
		...
		var p50FTPtr *float64
		if len(agg.firstTokenSamples) > 0 {
			v := computeP50Ms(agg.firstTokenSamples)
			p50FTPtr = &v
		} else if agg.firstTokenCount > 0 {
			// DB-restored windows only keep count/sum, so historical TTFT currently
			// falls back to mean until we persist raw samples or a quantile sketch.
...
// computeP50Ms returns the median of ns-duration samples converted to milliseconds.
func computeP50Ms(ns []int64) float64 {
	n := len(ns)
	if n == 0 {
		return 0
	}
	...
	if n%2 == 1 {
		return float64(sorted[n/2]) / 1e6
	}
	return float64(sorted[n/2-1]+sorted[n/2]) / 2 / 1e6
}
